...
class NonparametricRegressor(RegressorABC):
    ...
    def predict(self, abscissa: Union[Real, Sequence[Real]]) -> list:
        if self._abscissa is None or self._ordinate is None:
            raise RuntimeError("Fit must ba called first")
    ...
    def _get_approximation(self, abscissa:Real) -> float:
        ...
        numerator=sum(
            map(lambda tup_: tup_[0]*tup_[1], zip(self._ordinate, weights))
        )
    ...
    def _get_weights(self, abscissa: Real, window_size:float) -> list[float]:
        ...
        for abscissa_i in self._abscissa:
            kernel_argument=abs(abscissa-abscissa_i)/window_size  #ядро
            weight = 0,75*(1-kernel_argument)**2 if kernel_argument <1 else 0
