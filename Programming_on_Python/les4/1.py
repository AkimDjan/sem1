...
class PersonDB:
    ...
    def _good_login(self, login) -> bool:
        for i in login:
            if i not in self.check:
                return False
        if (login in self._login_registry) or login=='':
            return False
        return True
    
    def _good_password(self, password) -> bool:
        good_len=10
    ...
    def create_person(self, person: Person) -> UUID:
        ...
        if self._good_login(person.login) and self._good_password(person.password):
            uniq_id=uuid4()
            self._login_registry|={person.login}
            self._database[uniq_id]=copy_person
            return uniq_id
        else:
            raise ValueError
    ...
    def update_person_info(self, person_id: UUID, person_info_new: Person) -> None:
        ...
        if (person_info_new.login in self._login_registry):
            raise ValueError
        if person_info_new.login!='' and self._good_login(person_info_new.login):
        ...
        if person_info_new.password !='' and self._good_password(person.password):
